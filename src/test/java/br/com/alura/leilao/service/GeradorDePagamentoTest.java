...
class GeradorDePagamentoTest {
    ...
    @Test
    public void deveriaCriarPagamentoParaVencedorDoLeialao() {
        Leilao leilao = leilao();
        Lance vencedor = leilao.getLanceVencedor();


        LocalDate data = LocalDate.of(2021,8,20);
        ...
        Mockito.verify(pagamentoDao).salvar(captor.capture());  //Captor, capturar determinado objeto que foi criado e fazer as assertivas

        Pagamento pagamento = captor.getValue();

        Assertions.assertEquals(LocalDate.now().plusDays(3), pagamento.getVencimento());
